...
interface NewsCacheEntry {
  ...
  /** 真正的回傳資料。 */
  data: RelevantNewsItem[];
}

/** fundamentals cache。 */
const FUND_CACHE = new Map<string, FundCacheEntry>();
/** 成功 → 基本面預設 6 小時更新一次。 */
const FUND_TTL_OK = 6 * 60 * 60 * 1000;
/** 失敗 → 失敗時基本面只 cache 60 秒。 */
const FUND_TTL_FALLBACK = 60 * 1000;
...
export async function fetchStockRange(
  ...
  // 明確轉成 [key, string][]
  const qs = new URLSearchParams(safeEntries.map(([k, v]) => [k, String(v)])).toString();
...
// ===============================
export function normalizeStockRows(symbol: string = "", rows: StockLikeRow[] = []): StockBar[] {
  /** 將可能的數值資料正規化為 `number`，無效值則回傳 `null`。 */
  ...
  return rows
    .map((r) => {
      ...
      const adjClose = r.adjClose != null ? toNum(r.adjClose) : close;
...
// ===============================
export async function fetchSymbolProfile(
...
export async function fetchCompanyRank(codeOrSymbol: number | string): Promise<CompanyRank | null> {
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(`排名 API 錯誤：${res.status}`);
    }
...
export async function fetchMarketTreemapData({
  market = "TWSE",
}: { market?: MarketCode } = {}): Promise<MarketTreemapData> {
  const res = await fetch(api("/api/market-ranks"));

  if (!res.ok) {
    throw new Error(`市場市值 API 錯誤：${res.status}`);
  }
  ...
  const children: MarketTreemapNode[] = list
    .filter((r) => r.symbol.length > 0) // 基本防呆
    .map((r) => {
...
export async function fetchFundamentals(
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(`基本面 API 錯誤：${res.status}`);
    }
...
export async function fetchRelevantNews(
  ...
  if (!res.ok) {
    throw new Error(`新聞 API 錯誤：${res.status}`);
  }
